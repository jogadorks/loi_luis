...
limitawal = 30
memberlimit = 0
cr = '*✔️ESTE BOT ESTÁ VERIFICADO✔️*'
/*************************************/

/******** OWNER NUMBER**********/
const ownerNumber = ["5517981119174@s.whatsapp.net"] 
/************************************/
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Ola👋 @${num.split('@')[0]}\seja bem vindo ao grupo *${mdata.subject}* que se sinta casa aqui😀`
				...
				teks = `finalmente a carga do grupo é reduzida 𝟭, tchau    @${num.split('@')[0]} jseus serviços serão enterrados profundamente`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
       ...
       //	}
            ...
            case 'bitly':
               ...
               if (data.error) return reply(data.error)
                ...
                if (umurUser > 40) return reply(`Voce é muito velha no máximo 40 anos`)
                if (umurUser < 12) return reply(`Voce é muito jovem, mínimo 12 anos`)
                veri = sender
                if (isGroup) {
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("|")[1];
					const pref = `Uso: \n${prefix}quotemaker teks|watermark\n\nEx :\n${prefix}quotemaker aqui está um exemplo|bicit`
					if (args.length < 1) return reply(pref)
					reply(ind.wait())
					anu = await fetchJson(`https://terhambar.com/aw/qts/?kata=${quote}&author=${wm}&tipe=random`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {caption: 'Aqui esta🤙 ', quoted: mek})
					await limitAdd(sender)
					break
				//fadli 
				case 'pinterest':
				...
				case 'quotes':
					...
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                    if (args.length < 1) return reply(`Insira o número\nExemplo : ${prefix}infonomor 0812345678`)
...
• Do utilizador : @${updated.id.replace('@s.whatsapp.net','')}
• Limite: ${limitawal-updated.limit}`
            ...
            break
                ...
                if (jumblah < 100 ) return reply(`transferência mínima de 100`)
				...
				if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('enviar foto com percepção ${prefix}𝗼𝗰𝗿')
					}
					await limitAdd(sender)
				break
				case 'stiker': 
               ...
               client.sendMessage(from, resul, text, { quoted: mek})
				...
				case 'mining':
                      ...
                      await reply(`*Parabéns* ${pushname} você pegou *${mining}Xp*`)
                      }
                    await limitAdd(sender)
					break
				case 'grup':
           ...
           case 'demote':
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `parabéns por se tornar um administrador do grupo (+_+) :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`Parabéns🥳 @${mentioned[0].split('@')[0]} *por se tonar um administrador do grupo* (+_+)`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
				case 'listadmin':
				...
				case 'welcome':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (args.length < 1) return reply('0 ou 1? :𝘃')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('*ATIVADO* !!!')
						welkom.push(from)
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						reply('❬ sucesso ❭  ativar o recurso de boas-vindas/adeus neste grupo')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						reply('❬ sucesso ❭ desabilitar o recurso de boas-vindas/adeus neste grupo')
					} else {
						reply(ind.satukos())
					}
					break 
				case 'nsfw':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ sucesso ❭ ativado recursos nsfw neste grupo')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						reply('❬ sucesso ❭ desativado recursos nsfw neste grupo')
					} else {
						reply(ind.satukos())
					}
				break
				...
				case 'setpp': 
                       ...
                       media = await client.downloadAndSaveMediaMessage(mek)
                        ...
                        reply('GRUPO DE PERFIL DE ALTERAÇÃO DE SUCESSO')
					break				
				case 'leave': 
				if (!isGroup) return reply(ind.groupo())
				if (!isOwner) return reply(ind.ownerb())
